...
**有消息经过im服务端，可能会触发两个方法**

1. 处理发给社媒的客服消息：java客户端，im框架的监听消息方法onRecieveMessage，会处理收到来自app/web端客服的消息，调用各个社媒渠道发送消息方法，发给用户。(社媒渠道是在java客户端登录，收到消息也是在java客户端；app/web端的访客在前端登录，收到的客服消息是在前端有方法触发收到)
2. 处理用户消息：im服务端收到消息后发送一条mq消息，java客户端消费主题为im_save_msg的消息，判断是社媒的消息，就走转人工或意图，是坐席的就不处理（这个消费者会收到客服或者访客的，对于消费者来说都是用户，没有区分）
...
2. im服务端收到后，已经知道此用户在im中的用户id，通过ws转发给这个用户（估计是存了个map，保存用户id和ws连接关系）
   ...
   2.2 社媒渠道用户：呼叫中心-im模块（客户端），有方法监听到onRecieveMessage，收到消息后，在调用各自的社媒渠道发送方法，就能发过去
